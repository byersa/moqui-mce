...
const parseArgs = async () => {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '-h' || arg === '--help') {
            ...
            if (i + 1 < args.length) {
                ...
                console.error('Error: Host option requires a value');
                showHelp();
                process.exit(1);
            }
        } else if (arg === '--config') {
    ...
    return { port, host, quit, newToken, cleanTokens, encodedPair, daemon, startMCP };
};
...
const main = async () => {
    ...
    const HOST_BIND = CONFIG.host;
    httpServer.listen(PORT, HOST_BIND, () => {
        console.error(`WebSocket server running at http://${HOST_BIND}:${PORT}`);
        console.error(`WebSocket server running at http://${HOST}:${PORT}`); // HOST is still 'localhost' for display
